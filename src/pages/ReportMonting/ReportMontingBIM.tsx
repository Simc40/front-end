...
import { ForgePiece, Row } from "./ReportMonting";
...
export const ReportMontingBIM = ({
    ...
    const showNotification = (message: string) => {
        const overlay = document.getElementById("overlay2") as HTMLElement;
    ...
    const getCreationDate = (newDate: string) => {
            ...
            .then(
                (_retValue: {
                    ...
                    pieces.forEach((piece) => {
                        ...
                        if (
                            etapaIndex >= 8 &&
                            piece.pieceInfo?.etapas?.montagem !== undefined &&
                            getCreationDate(
                                piece.pieceInfo?.etapas?.montagem?.creation
                            ) <= referenceDate
                        )
                            color = new THREE.Color(0x96a1a2);
                        else if (
                            etapaIndex >= 7 &&
                            piece.pieceInfo?.etapas?.descarga !== undefined &&
                            getCreationDate(
                                piece.pieceInfo?.etapas?.descarga?.creation
                            ) <= referenceDate
                        )
                            color = new THREE.Color(0x00f700);
                        else if (
                            etapaIndex >= 6 &&
                            piece.pieceInfo?.etapas?.carga !== undefined &&
                            getCreationDate(
                                piece.pieceInfo?.etapas?.carga?.creation
                            ) <= referenceDate
                        )
                            color = new THREE.Color(0xfefe00);
                        else if (
                            etapaIndex >= 5 &&
                            piece.pieceInfo?.etapas?.liberacao !== undefined &&
                            getCreationDate(
                                piece.pieceInfo?.etapas?.liberacao?.creation
                            ) <= referenceDate
                        )
                            color = new THREE.Color(0x0e10ec);
                        else if (
                            etapaIndex >= 0 &&
                            piece.pieceInfo?.etapas?.cadastro !== undefined &&
                            getCreationDate(
                                piece.pieceInfo?.etapas?.cadastro?.creation
                            ) <= referenceDate
                        )
                            color = new THREE.Color(0x0ea8d4);
    ...
    const onScriptLoaded = () => {
        const preview = document.getElementById("preview2") as HTMLElement;
