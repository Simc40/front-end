...
import { Row } from "./ReportMonting";
import { DivProgressbar, DivProgressbarText } from "./MontingStyles";
        ...
        viewer.model
            ...
            .then(
                (_retValue: {
                    ...
                    const mapTableRows = new Map<string, number>(
                        tableRows.map((row, i) => {
                            return [row.nome_peca, i];
                        })
                    );
                    ...
                    Array.from(all_elementos.entries()).map(
                        ([dbId, nome_peca]: [number, string]) => {
                            if (mapTableRows.get(nome_peca) !== undefined)
                                tableRows[mapTableRows.get(nome_peca)!].dbId =
                                    dbId;
                            else {
                    ...
                    tableRows.forEach((row) => {
                        if (row.dbId === undefined) return;
                        if (
                            row.endMontagem !== undefined &&
                            row.endMontagem <= referenceDate
                        ) {
                            const color = new THREE.Color(0x96a1a2);
                            ...
                            row.startMontagem !== undefined &&
                            row.startMontagem <= referenceDate
                        ) {
                            const color = new THREE.Color(0x00f700);
                            ...
                            row.endTransporte !== undefined &&
                            row.endTransporte <= referenceDate
                        ) {
                            const color = new THREE.Color(0x00f700);
                            ...
                            row.startTransporte !== undefined &&
                            row.startTransporte <= referenceDate
                        ) {
                            const color = new THREE.Color(0xfefe00);
                            ...
                            row.endProducao !== undefined &&
                            row.endProducao <= referenceDate
                        ) {
                            const color = new THREE.Color(0x0e10ec);
                            ...
                            row.startProducao !== undefined &&
                            row.startProducao <= referenceDate
                        ) {
                            const color = new THREE.Color(0x0ea8d4);
