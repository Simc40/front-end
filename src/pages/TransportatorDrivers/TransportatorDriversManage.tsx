...
export const TransportatorDriversManage = ({drivers, uid_transportadora} : {drivers:Driver[] | undefined, uid_transportadora?:string}) => {
    ...
    useEffect(() => {
        auth.setPath("Home → Gerenciamento → Transportadoras → Motoristas → Gerenciar Motoristas")
    ...
    const columns: GridColDef[] = [
        { field: 'nome_motorista', headerName: 'Veículo', flex:2 , minWidth: 150},
    ...
    const onSubmit = () => {
        ...
        console.log(result)
        transportatorsApi().updateDriver(result)
        .then(async () => {
            setIsLoading(false)
            return swal({
                icon: 'success',
        ...
        return (
            <form onSubmit={handleSubmit(onSubmit as any)}>
                <Section text='Gerenciar Status de Clientes.'>
                    ...
                    <TableManage columnRows={columns} tableRows={drivers} callback={handleRowEditCommit}/>
                </Section>
