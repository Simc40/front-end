...
import { PDF, PDFConstructionApiGetInterface } from "../../types/Pdf";
import { CheckboxInput } from "../../components/Button/CheckboxInput";
import { Link } from "../../components/Links/Link";
import { pdfApi } from "../../apis/PdfApi";


export const PdfsConstructionPage = ({constructions, pdfs} : {constructions?:Construction[], pdfs?: PDFConstructionApiGetInterface}) => {
    ...
    useEffect(() => {
        auth.setPath("Home → Gerenciamento → PDFs → Obras")
    ...
    const onSubmit = () => {
        ...
        pdfApi().updatePdfConstruction(formData)
        .then(async () => {
            setIsLoading(false)
            return swal({
                icon: 'success',
    ...
    const onOptionSelected = (event: ChangeEvent<HTMLSelectElement>) => {
        ...
        if(pdfs![event.target.value] === undefined){
            setRows([]);
            setRowsCopy([]);
            return;
        }
        const selectedRows = Array.from(Object.values(pdfs![event.target.value]).map((pdf, i) => {return {...pdf, 'index': i}}));
    ...
    const constructionSelection = (constructions === undefined) ? [] : constructions.map((construction: Construction) => {return [construction.uid, construction.nome_obra]});
