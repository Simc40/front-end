...
export const ConstructionsManage = ({constructions} : {constructions:Construction[] | undefined}) => {
    ...
    useEffect(() => {
        auth.setPath("Home → Gerenciamento → Obras → Gerenciar Obras")
    ...
    const columns: GridColDef[] = [
        { field: 'nome_obra', headerName: 'Nome da Obra', flex:2 , minWidth: 150},
    ...
    const onSubmit = () => {
        ...
        console.log(result)
        constructionApi().updateConstruction(result)
        .then(async () => {
            setIsLoading(false)
            return swal({
                icon: 'success',
        ...
        return (
            <form onSubmit={handleSubmit(onSubmit as any)}>
                <Section text='Gerenciar Status de Clientes.'>
                    ...
                    <TableManage columnRows={columns} tableRows={constructions} callback={handleRowEditCommit}/>
                </Section>
