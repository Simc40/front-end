...
export const elementsApi: apiElementsInterface = () => ({
    getElements: async () => {
        const response: getElementsInterface = {
            elements: [],
            shapes: [],
            geometries: [],
            constructions: [],
        };
        await constructionApi()
            .getConstructions()
            .then((constructions: Construction[]) => {
                response.constructions = constructions;
            })
            .then(geometriesApi().getGeometries)
            .then((geometries: Geometry[]) => {
            ...
            .then(getRawElements)
            .then((elements) => {
                const elementsArray: Element[] = Array.from(
                    Object.values(elements).flatMap((elementDict) =>
                        Object.values(elementDict)
                    )
                );
                elementsArray.forEach((element) => {
                    element.construction = response.constructions.filter(
                        (construction) => construction.uid === element.obra
                    )[0];
                    element.shape = response.shapes.filter(
                        (shape) => shape.uid === element.forma
                    )[0];
                    element.geometry = response.geometries.filter(
                        (geometry) => geometry.uid === element.tipo
                    )[0];
                });
    ...
    getElementsMap: async () => {
        const response: getElementsMapInterface = {
            elements: new Map<string, Element>(),
            shapes: new Map<string, Shape>(),
            geometries: new Map<string, Geometry>(),
            constructions: new Map<string, Construction>(),
        };
        await constructionApi()
            .getConstructions()
            .then((constructions: Construction[]) => {
                constructions.forEach((construction) => {
                    response.constructions.set(construction.uid, construction);
                });
            })
            .then(geometriesApi().getGeometries)
            .then((geometries: Geometry[]) => {
                geometries.forEach((geometry) => {
                    response.geometries.set(geometry.uid, geometry);
                });
            })
            .then(shapesApi().getShapes)
            .then((shapes: Shape[]) => {
                shapes.forEach((shape) => {
                    response.shapes.set(shape.uid, shape);
                });
            })
            .then(getRawElements)
            .then((elements) => {
                ...
                elementsArray.forEach((element) => {
                    element.construction = response.constructions.get(
                        element.obra
                    );
...
const getRawElements = () => {
    return new Promise<{
        [uid_obra: string]: { [uid_elemento: string]: Element };
